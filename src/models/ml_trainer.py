...
# ── 서드파티 ──────────────────────────────────────────────────
import joblib  # 모델 직렬화. pickle 보다 sklearn 생태계 표준 + S301 회피
import numpy as np
import pandas as pd
...
def build_model(kind: ModelKind, params: dict[str, Any]):
    ...
    NOTE: XGBoost sklearn API 1.6+ 에서는 early_stopping_rounds 가 생성자 인자임
    (fit() 인자 방식은 deprecated). 따라서 호출자가 params 에 포함시켜 넘겨야 함.
    """
...
def cross_validate_model(
    ...
    *** 핵심 규칙: SMOTE 는 train fold 에만 적용 ***
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        # ── 모델 빌드 ────────────────────────────────────────
        # XGBoost sklearn API 1.6+: early_stopping_rounds 는 estimator 생성자
        # 인자로 전달해야 활성화된다 (fit() 인자 방식은 deprecated).
        # LightGBM 은 lgb.early_stopping callback 으로 처리 (아래 fit_kwargs).
        # 이 분기 안 걸면 XGBoost early stopping 이 조용히 무시됨.
        ...
        # ── fit() 호출용 kwargs 구성 ────────────────────────
        # XGB 와 LGBM 의 early stopping API 가 달라서 모델별 분기.
        fit_kwargs: dict[str, Any] = {}
        if early_stopping_rounds:
            if kind == "xgboost":
                # XGB: eval_set 만 fit 에 전달 (early_stopping_rounds 는 위에서 처리됨)
...
def fit_final_and_evaluate(
    ...
    NOTE: 여기는 eval_set 없이 학습하므로 early_stopping_rounds 가 params 에 들어
    있으면 안 됨 (XGBoost 가 'early_stopping_rounds 설정됐는데 eval_set 없음' 에러
    낼 수 있음). main_train.py 에서 default_params 만 넘기므로 안전.
    """
...
def save_model(model: Any, path: str | Path) -> None:
    ...
    pickle 대신 joblib 사용 이유:
      1. 보안: pickle.load 는 임의 코드 실행 위험 (Ruff S301 / Bandit B301).
         joblib 도 내부적으로 pickle 을 쓰지만, sklearn 생태계 표준이라 정적 분석
         도구가 예외 처리하며, 신뢰 영역 외 파일 로드 가능성을 낮추기 위한 모범 관례.
      2. 효율: numpy array 가 큰 모델(트리 앙상블) 직렬화 시 더 빠르고 파일 크기 작음.
      3. sklearn 호환: predict / predict_proba 인터페이스 그대로 보존.

    그럼에도 불구하고 *신뢰할 수 없는 출처의 모델 파일은 절대 로드하지 말 것*.
    """
    p = Path(path)
...
def load_model(path: str | Path) -> Any:
    """joblib 으로 모델 로드. save_model 의 짝.

    보안 주의: 이 함수는 *본인이 학습/저장한 신뢰 가능한 모델 파일*만 로드한다는
    가정 하에 동작한다. 외부에서 받은 .pkl/.joblib 파일은 절대 로드하지 말 것
    (joblib 내부의 pickle 역직렬화로 임의 코드가 실행될 수 있음).
    """
