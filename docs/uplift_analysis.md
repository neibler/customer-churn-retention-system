...
> 두 모델 모두 음수인 것은 시뮬레이터 small 모드(180일) 기준 처치 효과 신호가 약하기 때문이다. **full 모드(450일, 20,000명)** 실행 시 신호 강화 예상.
...
2. **full 모드 재실행 필수**: small(180일) 대비 full(450일, 20,000명)에서 신호 강화 예상
